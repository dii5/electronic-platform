...
echo'
    ...
    <ul style="list-style-type: none; height:50px;"> 
       <li><a href="deg_c++.php" class="decoration">عرض معلومات الطلاب</a></li>
    </ul>
<hr>
...
<h5 class="font-family m-2">اسألة الطلاب</h5>
 ...
 $show_com=$database->prepare(" SELECT *FROM comment_c ORDER BY id DESC");
  $show_com->execute();
  $i=0;
...
<div  class="container" style="width:80%;border-radius:10px;border:1px solid #E8E8E8;margin:auto;margin-top:30px !important;background:#055052;color:white; margin-bottom:30px;">
  ...
  if(isset($_POST['submit1'])){
      ...
      $add_file=$database->prepare("INSERT INTO class_noor (title,vidue, namefile, typefile,filedata)
      VALUES ('$title', '$vidue', '$fileName', '$fileType',':filedata')");
...
<div  class="container" style="width:80%;border-radius:10px;border:1px solid #E8E8E8;margin:auto;margin-top:10px !important;background:#055052;color:white;">
   <p class="m-2" style="font-family:cursive; font-size:25px;">فتح واغلاق تسجيل الحضور</p>';
       ...
       if(isset($_POST['open'])){
        $open_close=$database->prepare("UPDATE attend_c SET open_close ='1';");
        $open_close->execute();
       }
       if(isset($_POST['close'])){
        $open_close=$database->prepare("UPDATE attend_c SET open_close ='0';");
        $open_close->execute();
       }
   echo'
...
<div  class="container" style="width:80%;border-radius:10px;border:1px solid #E8E8E8;margin:auto;margin-top:10px !important;background:#055052; color:white;">
  ...
  $notes=$_POST['message'];
  $send_message=$database->prepare("INSERT INTO note(note_noor)VALUES('$notes')");
