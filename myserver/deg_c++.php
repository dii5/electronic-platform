...
<div style="font-family: cursive;">';
    ...
    echo '
      ...
      $show_day=$database->prepare("SELECT SUM(open_close)  FROM attend_c WHERE id_stu=$de[$i]");
      $show_day->execute();
    ...
    <button style="width:150px;"type="submit"name="submit2" class="btn btn-success m-2 mb-3">حفظ</button>
     ...
     if(!empty($_POST['a'.$a])){
      $newdeg=$_POST['a'.$a];      
      $open_close=$database->prepare("UPDATE inf_stu SET deg_c=$newdeg where id= $de[$a]");
      $open_close->execute();
      echo"<script>location.replace('deg_c++.php')</script>";
