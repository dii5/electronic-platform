...
<div class="row" style="width:700px; margin:auto">
   ...
   ';
    ...
    if(isset($_POST['submit1'])){
      ...
      $day=date('Y-m-d');
            $show=$database->prepare("SELECT * FROM attend_c WHERE id_stu=999999");
             $show->execute();
             $show=$show->fetchObject();
             
            if($show->open_close==1){
              if(empty($_COOKIE["day"])){
            $open_close=$database->prepare("INSERT INTO attend_c(open_close,id_stu,day)VALUES('1','$myid','$day')  " );
            $open_close->execute();
  ...
  ';
    if(isset($_POST['submit3'])){  
      ...
      $add_comn=$database->prepare("INSERT INTO comment_c (comment,id) VALUES('$comment','')"); 
      $add_comn->execute();}}

$show3=$database->prepare("SELECT * FROM class_noor ORDER BY id DESC");
$show3->execute();
