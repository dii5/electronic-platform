...
<div class="row" style="width:700px; margin:auto">
  ...
  $database=new PDO("mysql:host=localhost;dbname=school;charset=utf8;",$user,$pass);
    ...
    if(isset($_POST['submit1'])){
       $day=date('Y-m-d');
            $show=$database->prepare("SELECT * FROM attend_c WHERE id_stu=9999");
             $show->execute();
             $show=$show->fetchObject();
            if($show->open_close==1){
              if(empty($_COOKIE["day"])){
            $open_close=$database->prepare("INSERT INTO attend_dir(open_close,id_stu,day)VALUES('1','$myid','$day')  " );
            if($open_close->execute()){
  ...
  <button type="submit"style=" width:50%;" class="btn btn-primary col-3 m-1" name="submit1">تسجيل</button>
  </div>
    </form>
</div>
    <!-- days end-->
  ...
  ';
    if(isset($_POST['submit3'])){  
      ...
      $add_comn=$database->prepare("INSERT INTO comment_dir (comment) VALUES('$comment')"); 
      $add_comn->execute();}}
    

$show3=$database->prepare("SELECT * FROM class_dir ORDER BY id DESC");
$show3->execute();
           ...
           <div class="row g-0">
            ...
            echo'<div class="col-6" style="height:60px; border-radius:3px;border:1px solid #EEEEEE;">
                  ...
                  <div class="col-9">';
                      ...
                      $getdata ="data:".$show2['typefile'].";base64,".base64_encode($show2['filedata']);
                      
                      
                     echo' <a href="'.$getdata.'" download><p  style="overflow:hidden;height:58px;">'.$show2['namefile'].'</p></a>
                       </div>
                      </div>
                    </div>';}
                    echo'
